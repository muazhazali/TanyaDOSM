...
import { AlertCircle, BarChart3, Check, ChevronDown, Circle, Clock3, Copy, Database, Download, ExternalLink, FileQuestion, LoaderCircle, Menu, Pencil, RefreshCw, Send, Share2, ThumbsDown, ThumbsUp, Trash2, X } from 'lucide-react'
...
function StatusPill({ status }: { status: RunStatus }) {
  const labels: Record<RunStatus, string> = { queued: 'Waiting', running: 'Working', completed: 'Complete', failed: 'Needs attention', interrupted: 'Stopped' }
...
function ResultTable({ answer }: { answer: AnswerPayload }) {
  ...
  return <details className="rounded-2xl border border-slate-200 bg-white" open={rows.length <= 8}>
...
function Results({ answer, runId, question, onFollowUp }: { answer: AnswerPayload; runId: string; question: string; onFollowUp?: (value: string) => void }) {
  ...
  const copy = async (kind: 'answer' | 'link') => {
    const value = kind === 'answer' ? answer.answer : `${window.location.origin}${window.location.pathname}?run=${runId}`
  ...
  return <section className="space-y-5" aria-live="polite">
    <div className={`rounded-2xl border p-5 ${answer.error ? 'border-amber-200 bg-amber-50' : 'border-emerald-200 bg-gradient-to-br from-emerald-50 to-white'}`}>
      ...
      {headline && !answer.error && <div className="mt-3"><p className="text-4xl font-bold tracking-tight text-emerald-900">{headline.value}</p><p className="mt-1 text-sm font-medium capitalize text-emerald-700">{headline.label}</p></div>}
      <p className={`${headline ? 'mt-4 text-base' : 'mt-2 text-lg'} leading-relaxed text-slate-900`}>{answer.answer}</p>
    </div>
    ...
    {answer.source && <div className="rounded-2xl border border-slate-200 bg-white p-4 text-sm text-slate-600">
      ...
      <p className="mt-1">{answer.source.agency} / Period: {answer.source.period ? formatCell(answer.source.period) : 'Not specified'} / Unit: {answer.source.unit}</p>
      <p className="mt-1 text-xs text-slate-500">Based on {rowsUsed} verified {rowsUsed === 1 ? 'row' : 'rows'} from the selected dataset.</p>
    </div>}
...
function ProcessDetails({ events, snapshot }: { events: RunEvent[]; snapshot?: RunSnapshot }) {
  ...
  return <details className="mt-6 rounded-2xl border border-slate-200 bg-white p-4"><summary className="flex cursor-pointer list-none items-center justify-between font-semibold text-slate-800">How we found this answer <ChevronDown size={18} /></summary><p className="mt-2 text-sm text-slate-500">Follow the path from your question to checked official data.</p><Timeline events={events} />{(artifacts.some(([, type]) => latestArtifact(events, type)) || snapshot?.answer?.trace) && <details className="mt-4 rounded-xl border border-slate-200 bg-slate-50 p-3"><summary className="cursor-pointer text-sm font-semibold text-slate-700">Technical details</summary><p className="mt-2 text-xs text-slate-500">Structured records used to produce the answer. Prompts and private reasoning are never shown.</p><div className="mt-3 space-y-2">{artifacts.map(([label, type]) => { const artifact = latestArtifact(events, type); return artifact ? <details key={type} className="rounded-lg border bg-white p-3"><summary className="cursor-pointer text-xs font-semibold">{label}</summary><pre className="mt-3 overflow-x-auto whitespace-pre-wrap text-xs text-slate-600">{JSON.stringify(artifact.payload, null, 2)}</pre></details> : null })}</div></details>}</details>
}

function DatasetGuide({ datasets, loading, onChoose }: { datasets?: DatasetDefinition[]; loading: boolean; onChoose: (question: string) => void }) {
  const exampleFor = (dataset: DatasetDefinition) => dataset.geography_level === 'state' ? `Compare Johor and Selangor ${dataset.measures[0]?.name.replaceAll('_', ' ') || dataset.domain}.` : `Show Malaysia's latest ${dataset.measures[0]?.name.replaceAll('_', ' ') || dataset.domain}.`
  return <details className="mt-8 rounded-2xl border border-slate-200 bg-white p-5"><summary className="flex cursor-pointer list-none items-center justify-between"><span><span className="flex items-center gap-2 font-bold"><Database size={18} className="text-emerald-700" /> What can I ask?</span><span className="mt-1 block text-sm font-normal text-slate-500">Browse the five curated official datasets and example questions.</span></span><ChevronDown /></summary>{loading && <p className="mt-4 text-sm text-slate-500">Loading available data…</p>}<div className="mt-4 grid gap-3 sm:grid-cols-2">{datasets?.map((dataset) => <article key={dataset.dataset_id} className="rounded-xl border border-slate-200 p-4"><h4 className="font-semibold">{dataset.title}</h4><p className="mt-1 text-xs capitalize text-slate-500">{dataset.geography_level} · {dataset.frequency}</p><p className="mt-2 text-sm text-slate-600">{dataset.description}</p><p className="mt-2 text-xs text-slate-500">Measures: {dataset.measures.map((measure) => measure.name.replaceAll('_', ' ')).join(', ')}</p>{dataset.caveats.length > 0 && <p className="mt-2 text-xs text-amber-700">Note: {dataset.caveats[0]}</p>}<button onClick={() => onChoose(exampleFor(dataset))} className="mt-3 text-left text-sm font-semibold text-emerald-700 hover:underline">Try: “{exampleFor(dataset)}”</button></article>)}</div></details>
}
  ...
  return <div className="min-h-screen bg-slate-50 text-slate-900">
    ...
    <div className="grid min-h-screen lg:grid-cols-[280px_minmax(0,1fr)]">
      <aside className={`${sidebarOpen ? 'fixed inset-0 z-30 block' : 'hidden'} border-r border-slate-200 bg-slate-950 text-white lg:static lg:block`}><div className="flex h-full flex-col p-5 lg:sticky lg:top-0 lg:h-screen">
        <div className="flex items-center justify-between"><div className="flex items-center gap-3"><BarChart3 className="text-emerald-400" /><div><h1 className="text-xl font-bold">TanyaDOSM</h1><p className="text-xs text-slate-400">Official data, explained</p></div></div><button className="lg:hidden" aria-label="Close recent chats" onClick={() => setSidebarOpen(false)}><X /></button></div>
