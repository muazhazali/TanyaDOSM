...
test('reopens a completed persisted conversation', async ({ page }) => {
  ...
  await expect(page.getByText('The requested population is 34.2 million people.')).toBeVisible()
  await page.reload()
  await expect(page.getByText('Population of Malaysia')).toBeVisible()
  await expect(page.getByText('34.2 million', { exact: true })).toBeVisible()
